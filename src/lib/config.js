...
const raw = cleanEnv(process.env, {
    ...
    EGRESS_DETECT: bool({ default: true, desc: 'Discover public egress IPv4/IPv6 for the firewall allowlist (reported via X-Agent-Health)' }),
    EGRESS_DETECT_URL: url({ default: DEFAULT_EGRESS_DETECT_URL, desc: 'Dual-stack echo URL that returns the client IP (cdn-cgi/trace or plain text)' }),
    EGRESS_DETECT_TTL_MS: num({ default: EGRESS_DETECT_TTL_MS, desc: 'How long a discovered egress IP is reused before re-probing' }),
    EGRESS_DETECT_TIMEOUT_MS: num({ default: EGRESS_DETECT_TIMEOUT_MS, desc: 'Per-family timeout for egress discovery' }),
